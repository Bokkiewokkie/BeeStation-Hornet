...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			dat += "<b>Character Origin:</b><BR><a href = '?_src_=prefs;preference=origin;task=input'>[active_character.origin]</a><BR></td>"
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("origin")
					var/new_origin = input(user, "Choose your character's origin, this may change some gameplay aspects:", "Character Preference") as null|anything in active_character.pref_species.valid_origins
					if(new_origin)
						active_character.origin = new_origin
